...
async function inicializarDatosPrueba() {
  ...
  try {
    ...
    // ── Clientes ──────────────────────────────────────────────
    const clientes = [
      { nombre: 'Televisa Puebla', tipo: 'medio', sector: 'Televisión', contacto_nombre: 'Laura Martínez',
        email: 'produccion@televisapuebla.mx', telefono: '222-100-2000', ciudad: 'Puebla',
        estado: 'activo', notas: 'Canal regional con noticieros diarios.', created_at: ts },
      { nombre: 'DIF Estatal Puebla', tipo: 'gobierno', sector: 'Salud Social', contacto_nombre: 'Carlos Hernández',
        email: 'c.hernandez@difpuebla.gob.mx', telefono: '222-203-4000', ciudad: 'Puebla',
        estado: 'activo', notas: 'Dependencia gubernamental. Requiere LSM en eventos públicos.', created_at: ts },
      { nombre: 'UDLAP', tipo: 'escuela', sector: 'Educación Superior', contacto_nombre: 'Ana Torres',
        email: 'eventos@udlap.mx', telefono: '222-229-2000', ciudad: 'San Andrés Cholula',
        estado: 'prospecto', notas: 'Universidad privada. Interés en talleres de sensibilización.', created_at: ts },
      { nombre: 'Auditorio Metropolitano', tipo: 'evento', sector: 'Entretenimiento', contacto_nombre: 'Roberto Sánchez',
        email: 'rsanchez@auditoriometro.mx', telefono: '222-333-5000', ciudad: 'Puebla',
        estado: 'negociacion', notas: 'Organizan conciertos y obras de teatro. Piden cotización mensual.', created_at: ts },
      { nombre: 'Cruz Roja Puebla', tipo: 'comunidad', sector: 'Salud / ONG', contacto_nombre: 'María López',
        email: 'mlopez@cruzrojapuebla.org', telefono: '222-405-6000', ciudad: 'Puebla',
        estado: 'activo', notas: 'ONG de salud. Convenio anual en revisión.', created_at: ts }
    ];

    const clienteRefs = [];
    for (const c of clientes) {
      const ref = await db.collection(COL_CLIENTES).add(c);
      clienteRefs.push({ ref, nombre: c.nombre });
    }

    // ── Cotizaciones ─────────────────────────────────────────
    const serviciosMuestra = [
      { servicio_id: 'interpretacion_vivo', nombre: 'Interpretación en Vivo (Noticias / TV)',
        unidad: 'hora', precio_unitario: 800, cantidad: 4, subtotal: 3200 },
      { servicio_id: 'subtitulacion_vod', nombre: 'Subtitulación y Transcripción para VOD',
        unidad: 'minuto', precio_unitario: 25, cantidad: 60, subtotal: 1500 }
    ];
    const cotizaciones = [
      { numero: 'COT-0001', cliente_id: clienteRefs[0].ref.id, cliente_nombre: clienteRefs[0].nombre,
        modalidad: 'presencial', fecha_servicio: '2026-05-10', duracion: '4 horas',
        servicios: serviciosMuestra, total: 4700, estado: 'aceptada',
        notas: 'Cobertura especial elecciones locales.', creado_por: mmhUid, created_at: ts },
      { numero: 'COT-0002', cliente_id: clienteRefs[1].ref.id, cliente_nombre: clienteRefs[1].nombre,
        modalidad: 'presencial', fecha_servicio: '2026-05-20', duracion: '2 horas',
        servicios: [{ servicio_id: 'interpretacion_conferencia', nombre: 'Interpretación en Conferencias',
          unidad: 'hora', precio_unitario: 650, cantidad: 2, subtotal: 1300 }],
        total: 1300, estado: 'enviada',
        notas: 'Conferencia de prensa sobre programas sociales.', creado_por: mmhUid, created_at: ts },
      { numero: 'COT-0003', cliente_id: clienteRefs[2].ref.id, cliente_nombre: clienteRefs[2].nombre,
        modalidad: 'presencial', fecha_servicio: '2026-06-01', duracion: '8 horas',
        servicios: [{ servicio_id: 'capacitacion_basica', nombre: 'Capacitación LSM Básica',
          unidad: 'persona', precio_unitario: 250, cantidad: 20, subtotal: 5000 }],
        total: 5000, estado: 'borrador',
        notas: 'Taller para alumnos de nuevo ingreso.', creado_por: mmhUid, created_at: ts },
      { numero: 'COT-0004', cliente_id: clienteRefs[3].ref.id, cliente_nombre: clienteRefs[3].nombre,
        modalidad: 'presencial', fecha_servicio: '2026-04-25', duracion: '6 horas',
        servicios: [{ servicio_id: 'interpretacion_vivo', nombre: 'Interpretación en Vivo',
          unidad: 'hora', precio_unitario: 800, cantidad: 6, subtotal: 4800 }],
        total: 4800, estado: 'rechazada',
        notas: 'Cliente solicitó reducir precio. Sin acuerdo.', creado_por: mmhUid, created_at: ts },
      { numero: 'COT-0005', cliente_id: clienteRefs[4].ref.id, cliente_nombre: clienteRefs[4].nombre,
        modalidad: 'remoto', fecha_servicio: '2026-05-15', duracion: '3 horas',
        servicios: [{ servicio_id: 'interpretacion_online', nombre: 'Interpretación Remota / Streaming',
          unidad: 'hora', precio_unitario: 500, cantidad: 3, subtotal: 1500 }],
        total: 1500, estado: 'enviada',
        notas: 'Webinar de capacitación interna.', creado_por: mmhUid, created_at: ts }
    ];

    for (const cot of cotizaciones) {
      await db.collection(COL_COTIZACIONES).add(cot);
    }

    // ── Solicitudes de contacto ──────────────────────────────
    const solicitudes = [
      { nombre: 'Pedro Ramírez', empresa: 'Grupo Imagen Puebla', email: 'p.ramirez@imagenpuebla.mx',
        telefono: '222-500-1000', servicio_interes: 'interpretacion_vivo',
        mensaje: 'Necesitamos intérprete para nuestro noticiero de las 7pm, de lunes a viernes.',
        estado: 'nueva', fuente: 'formulario_web', created_at: ts },
      { nombre: 'Gabriela Morales', empresa: 'Ayuntamiento de Tehuacán', email: 'g.morales@tehuacan.gob.mx',
        telefono: '238-382-1000', servicio_interes: 'interpretacion_conferencia',
        mensaje: 'Buscamos intérprete para el informe de gobierno del próximo mes.',
        estado: 'revisada', fuente: 'formulario_web', created_at: ts },
      { nombre: 'Jorge Vega', empresa: '', email: 'jorge.vega@gmail.com',
        telefono: '222-600-7000', servicio_interes: 'capacitacion',
        mensaje: 'Me interesa un taller de LSM básico para mi equipo de 15 personas.',
        estado: 'nueva', fuente: 'formulario_web', created_at: ts }
    ];

    for (const s of solicitudes) {
      await db.collection(COL_SOLS).add(s);
    }

    showToast('Base de datos inicializada: 5 clientes, 5 cotizaciones, 3 solicitudes.');
    if (btn) { btn.textContent = 'Listo ✓'; btn.style.color = '#16a34a'; }
    cargarDashboard();
    iniciarListenerSolicitudes();

  } catch (err) {
    console.error('Error inicializando datos:', err);
    showToast('Error al crear datos de prueba: ' + err.message, 'error');
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="ph-database-fill"></i> Datos de prueba'; }
  }
}